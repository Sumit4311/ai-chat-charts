...
import { yearopt, getFileIcon } from 'app/services/CommonObject'
...
import { deleteDataFromApi, getDataFromApi, postDataFromApi } from '../../services/CommonService';
...
const ESG = () => {
    ...
    function handleCloseEsgDetails() {
        setOpen(false)
    ...
    const filterList = (filedata = '') => {
        ...
        if (formdata.searchYear) {
            ...
            let newSearchArr = []
            newfiledata.find((opt) => {
                if (opt?.category.trim() == formdata.searchcategory.trim()) {
                    newSearchArr.push(opt);
                }
            })
            // setFilteredArray(newSearchArr)
            ...
            let newSearchArr = []
            newfiledata.find((opt) => {
                if (opt?.searchYear == formdata.searchYear &&
                    opt?.documentType == formdata.searchdocumenttype.trim() &&
            ...
            let newSearchArr = []
            newfiledata.find((opt) => {
                if (opt?.companyId == formdata.searchcompany && opt?.searchYear == formdata.searchYear && opt?.category.trim == formdata.searchcategory && opt?.documentType == formdata.searchdocumenttype.trim() && opt?.quarter.trim() == formdata.period.trim()) {
                    newSearchArr.push(opt);
                }
            })
            // setCompleteFiles(newSearchArr)
            displayingrecords(newSearchArr)
        } else if (formdata.searchcompany && formdata.searchdocumenttype && formdata.searchYear && formdata.searchcategory) {
            let newSearchArr = []

            newfiledata.find((opt) => {
                if (opt?.companyId == formdata.searchcompany && opt?.searchYear == formdata.searchYear && opt?.documentType == formdata.searchdocumenttype && opt?.category == formdata.searchcategory) {
                    newSearchArr.push(opt);
                }
            })
            // setFilteredArray(newSearchArr)
        ...
        } else if (formdata.searchcompany && formdata.searchdocumenttype && formdata.period && formdata.searchYear) {
            let newSearchArr = []
            newfiledata.find((opt) => {
                if (opt?.companyId == formdata.searchcompany && opt?.searchYear == formdata.searchYear && opt?.documentType.trim() == formdata.searchdocumenttype.trim() && opt?.quarter.trim() == formdata.period.trim()) {
                    newSearchArr.push(opt);
                }
            })
            // setFilteredArray(newSearchArr)
            displayingrecords(newSearchArr)
        } else if (formdata.searchcompany && formdata.searchcategory && formdata.period && formdata.searchYear) {
            let newSearchArr = []
            newfiledata.find((opt) => {
                if (opt?.companyId == formdata.searchcompany && opt?.searchYear == formdata.searchYear && opt?.category.trim() == formdata.searchcategory.trim() && opt?.quarter.trim() == formdata.period.trim()) {
                    newSearchArr.push(opt);
                }
            })
            // setFilteredArray(newSearchArr)
            displayingrecords(newSearchArr)
        } else if (formdata.searchdocumenttype && formdata.searchcategory && formdata.period && formdata.searchYear) {
            let newSearchArr = []
            newfiledata.find((opt) => {
                if (opt?.documentType.trim() == formdata.searchdocumenttype.trim() && opt?.searchYear == formdata.searchYear && opt?.category.trim() == formdata.searchcategory.trim() && opt?.quarter.trim() == formdata.period.trim()) {
                    newSearchArr.push(opt);
                }
            })
            // setFilteredArray(newSearchArr)
            ...
            // setFilteredArray(newSearchArr)
            displayingrecords(newSearchArr)
        } else if (formdata.searchdocumenttype && formdata.period) {
            let newSearchArr = []
            newfiledata.find((opt) => {
                if (opt?.documentType.trim() == formdata.searchdocumenttype.trim() && opt?.quarter.trim() == formdata.period.trim()) {
                    newSearchArr.push(opt);
                }
            })
            // setFilteredArray(newSearchArr)
    ...
    const getFileNames = async () => {
        ...
        if (response?.status == 200 && response?.data) {
            // setnameDetails(response.data);
            // const ESGlist = response.data.filter(val => val.category === 'ESG' && val.companyId === "c68a7105-6bf2-4447-ad84-fd776713545c")
            // console.log('formdata', ESGlist);

            setFileArray(response?.data)
            // setFilteredArray(ESGlist)
        ...
        <Container>
                            ...
                            >
                                <div className="singlefilefol">
                                    ...
                                    <Icon fontSize="medium"
                                        ...
                                        onClick={(e) => handleClicksubmenu(e, file)} >more_horiz</Icon>
                                </div>
                            </Grid>
                        ))}
                    </Grid>
            ...
            >
                <ValidatorForm onSubmit={handleSubmit} className="customform" onError={() => null}>
                    <DialogTitle id="form-dialog-title" onClose={handleCloseEsgDetails}>Upload Documents</DialogTitle>
                    <DialogContent>
            ...
            >
                <DialogTitle id="form-dialog-title" onClose={handleCloseEsgDetails}>
                    Document preview
                </DialogTitle>
                <DialogContent>
                    <img src={getFileIcon(formdata.filename)} />
