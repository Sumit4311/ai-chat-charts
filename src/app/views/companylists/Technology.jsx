...
const Commercialbusiness = () => {
    ...
    const [formdata, setFormData] = useState({ yearCapex: currentyear, yearCos: currentyear, quaterCOS: "Q1", quaterCapex: "Q1", yearCosLine: currentyear, yearOpexLine: currentyear })
    ...
    const [isCosHighest, setIsCosHighest] = useState(false)
    const [isCosYTD, setIsCosYTD] = useState(false)
    ...
    const [tabDataValue, setTabDataValue] = useState({
        cosGraphopt: "cosGraphopt_0",
        capexGraphopt: "capexGraphopt_0",
        optionbycoshighest: "optionbycoshighest_0",
        ...
        baseOption: {
            ...
            xAxis: {
                ...
                axisLabel: {
                    color: '#fff',
                    rotate: 0,
                    show: true, // show y-axis even with no data
                },
                // show: true, // show x-axis even with no data
        ...
        series: [
            {
                ...
                itemStyle: {
                    color: function (params) {
                        return params.data[1] < 0 ? 'red' : '#49C9BD';
                    },
                },
            },
        ],
    ...
    useEffect(() => {
        getCosExHighestList()
    }, [formdata.yearCapex, currency, formdata.quaterCapex, companyId])

    useEffect(() => {
        getCosHighestList()
    }, [formdata.yearCos, currency, formdata.quaterCOS, companyId])
    useEffect(() => {
        getcostOfService()
    }, [formdata.yearCosLine, currency, companyId])
    useEffect(() => {
        getcapitalExpendituresGraph()
    }, [formdata.yearOpexLine, currency, companyId])

    const getCosHighestList = async () => {
        const response = await getDataFromApi(
            getCostOfServiceHighest + `year=${formdata.yearCos}&companyId=${companyId}&quarter=${formdata.quaterCOS}&convater=${convater}`,
            1
        )
        setIsCosHighest(false)
        if (response && response.status == 200 && Object.keys(response?.data?.monthsValue).length > 0) {
            ...
            const productName = ['product', response?.data?.productName]
            // setMonthList([['product', response.data.productName]])
            setMonthList([['product', response.data.productName], ...fill_empty_months_info])
            setTimeout(() => {
                setIsCosHighest(true)
            }, 10)
        } else {
    ...
    const getCosExHighestList = async () => {
        const response = await getDataFromApi(
        ...
        if (response && response.status == 200 && Object.keys(response?.data?.monthsValue).length > 0) {
            ...
            setProductName(response?.data?.productName)
            const productName = ['product', response?.data?.productName]
            // setcosExMonthList([productName, ...fill_empty_months_info])
    ...
    const getcostOfService = async () => {
        ...
        var year = currentyear
        const response = await getDataFromApi(
            getCostOfService + `companyId=${companyId}&year=${formdata.yearCosLine}`,
            1
        )
        if (response && response?.status == 200 && response?.data?.length > 0) {
            // costData.sort((a, b) => quarterOrder[a.quarter] - quarterOrder[b.quarter]);

            let costData=response?.data?.map((cosData,i)=>{
                if(cosData.value.length<=4){
                    if(cosData.value.findIndex(i=>i.quarter==='Q1')<0 &&cosData.value.findIndex(i=>i.productName=== cosData.productName)>-1){
                        cosData.value.push({'cValue' : null, 'productName' :  cosData.productName,'quarter' :  "Q1" ,'year' : null})
                    }
                    if(cosData.value.findIndex(i=>i.quarter==='Q2')<0 &&cosData.value.findIndex(i=>i.productName=== cosData.productName)>-1){
                        cosData.value.push({'cValue' : null, 'productName' :  cosData.productName,'quarter' :  "Q2" ,'year' : null})
                    }
                    if(cosData.value.findIndex(i=>i.quarter==='Q3')<0 &&cosData.value.findIndex(i=>i.productName=== cosData.productName)>-1){
                        cosData.value.push({'cValue' : null, 'productName' :  cosData.productName,'quarter' :  "Q3" ,'year' : null})
                    }
                    if(cosData.value.findIndex(i=>i.quarter==='Q4')<0 &&cosData.value.findIndex(i=>i.productName=== cosData.productName)>-1){
                        cosData.value.push({'cValue' : null, 'productName' :  cosData.productName,'quarter' :  "Q4" ,'year' : null})
                    }
                    return cosData
                }
            ...
            response.data.map((cos, i) => {
                ...
                if (!alreadyExistQuarter) {
                    cos.value.map((cosvalue, i) => {
                        var quarteryear = [cosvalue.quarter]
            ...
            var cosSelected = formdata.yearCosLine
            setcosGraphopt([
    ...
    const getcapitalExpendituresGraph = async () => {
        var query = ''
        var year = currentyear
        // Getdata
        ...
        let response = await getDataFromApi(
            getCapitalExpendituresGraph + '?companyId=' + `${companyId}&year=${formdata.yearOpexLine}`,
            1
        )
        ...
        if (response && response.status == 200 && response.data.length > 0) {
            ...
            let capexData=response?.data?.map((cosData,i)=>{
                if(cosData.value.length<=4){
                    if(cosData.value.findIndex(i=>i.quarter==='Q1')<0 &&cosData.value.findIndex(i=>i.productName=== cosData.productName)>-1){
                        cosData.value.push({'cValue' : null, 'productName' :  cosData.productName,'quarter' :  "Q1" ,'year' : null})
                    }
            ...
            // setcostOfService(capexData)
            setcapitalExpendituresGraph(capexData)
            ...
            response.data.map((capex, i) => {
                ...
                if (!alreadyExistQuarter) {
                    capex.value.reverse().map((capexvalue) => {
                        var quarteryear = [capexvalue.quarter]
            ...
            console.log(maincapexopt, "maincapexopt")
            ...
            var selectedOpexYear = formdata.yearOpexLine
            setcapexGraphopt([
                // ['product', 'BOOK', 'PRODUCTNAMEC', 'PEN'],
                ['Q1FY' + selectedOpexYear, 0],
                ['Q2FY' + selectedOpexYear, 0,],
                ['Q3FY' + selectedOpexYear, 0,],
                ['Q4FY' + selectedOpexYear, 0,],
            ])
            // }
    ...
    return (
        <Container>
            <div className="breadcrumb">
                <Breadcrumb routeSegments={[{ name: 'Technology' }]} />
            ...
            <Grid container spacing={3}>
                ...
                >
                    <StyledCard elevation={6} className="echarts paddingbottom">
                        <CardHeader className="cardheader">
                            <Title>CoS YTD and PER YEAR({currency})</Title>
                        </CardHeader>
                        <Tabs
                            value={tabDataValue.cosGraphopt}
                            onChange={(e, value) => handleTabsChange(value, 'cosGraphopt')}
                        ...
                        >
                            <Tab label="Graph" value={'cosGraphopt_0'} />
                            <Tab label="Table" value={'cosGraphopt_1'} />
                        </Tabs>
                            ...
                            <div className="year-form-inner">
                                ...
                                <AutoComplete
                                    ...
                                    onChange={(event, value) => {
                                        changedropdownvalue('yearCosLine', value?.label)
                                    }
                                    }
                                    renderInput={(params) => (
                                        <TextField
                            ...
                            >
                                <div className="table_scroll">
                                    <StyledTable className="customtable odd-even withborder">
                                        <TableHead>
                                            <TableRow>
                                            <TableCell style={{width:'150px'}}>Product</TableCell>
                                            <TableCell>Q1</TableCell>
                                            <TableCell>Q2</TableCell>
                                            <TableCell>Q3</TableCell>
                                            <TableCell>Q4</TableCell>
                                                {/* {
                                                    cosGraphopt.map((singleData) => (
                                        ...
                                        <TableBody>
                                            {
                                                cosGraphopt.length > 0 && cosGraphopt[0].map((singleData, index) => (
                                                    index != 0 && (
                                                        <TableRow>
                                                            <TableCell align="left" className="imgCell">
                                                                <span>
                                                            ...
                                                            {
                                                                // cosGraphopt.map((singleData, i) => (
                                                                    costOfService?.map((cosData, i) => (
                                                                        <>
                                                                       {cosData?.productName===singleData&&cosData.value.sort((a, b) => quarterOrder[a.quarter] - quarterOrder[b.quarter]).map((cosValue,i)=>(
                                                                        <>
                                                                        {  
                                                                        <TableCell align="left" className="imgCell">
                                                                        { 
                ...
                >
                    <StyledCard elevation={6} className="echarts paddingbottom">
                        <CardHeader className="cardheader">
                            <Title>Opex YTD and PER YEAR({currency})</Title>
                        </CardHeader>
                        <Tabs
                        ...
                        <ValidatorForm className="year-form">
                            <div className="year-form-inner">
                                ...
                                <AutoComplete
                                    ...
                                    getOptionLabel={(option) => option.label}
                                    onChange={(event, value) => {
                                        changedropdownvalue('yearOpexLine', value?.label)
                                    }
                                    }
                                    renderInput={(params) => (
                                        <TextField
                                                    ...
                                                    index != 0 && (
                                                        <TableRow>
                                                            ...
                                                            {/* {
                                                             ...
                                                             {
                                                                // cosGraphopt.map((singleData, i) => (
                                                                    capitalExpendituresGraph?.map((cosData, i) => (
                                                                        <>
                                                                       {cosData?.productName===singleData&&cosData.value.sort((a, b) => quarterOrder[a.quarter] - quarterOrder[b.quarter]).map((cosValue,i)=>(
                                                                        <>
                                                                        {  
                ...
                <Grid item lg={6} md={6} sm={12} xs={12} className="noPadding">
                    <StyledCard elevation={6} className="echarts paddingbottom">
                        <CardHeader className="cardheader">
                            <Title>Cos Highest ({currency})</Title>
                        </CardHeader>
                        <Tabs
                            value={tabDataValue.optionbycoshighest}
                            onChange={(e, value) => handleTabsChange(value, 'optionbycoshighest')}
                            aria-label="basic tabs example"
                            className="whitebg"
                            variant="scrollable"
                            scrollButtons="auto"
                        ...
                        >
                            <Tab label="Graph" value={'optionbycoshighest_0'} />
                            <Tab label="Table" value={'optionbycoshighest_1'} />
                        </Tabs>
                        <ValidatorForm className="year-form">
                            <div className="year-form-inner">
                                <label>Quarter:</label>
                                <AutoComplete
                                    ...
                                    getOptionLabel={(option) => option.label}
                                    onChange={(event, value) => {
                                        changedropdownvalue('quaterCOS', value?.label)
                                    }
                                    }
                                    renderInput={(params) => (
                                        <TextField
                        ...
                        <SimpleCard>
                            ...
                            <TabPanel style={{overflow:'auto'}}
                                value={tabDataValue.optionbycoshighest}
                                index={'optionbycoshighest_1'}
                                className="tabpanel nopadding notclear"
                            >
                                <div className="table_scroll" style={{width:'350%'}}>
                                    <StyledTable className="customtable odd-even withborder">
                                        <TableHead>
                                            <TableRow>
                                                {monthsList.map((singleData) => (
                                                    <TableCell align="left" className="imgCell">
                                                        <span>
                                                            {singleData[0]}
                                                        </span>
                                        ...
                                        <TableBody>
                                            {monthsList.length > 0 && monthsList[0].map((singleData, index) => (
                                                index != 0 && (
                                                    <TableRow>
                                                        <TableCell align="left" className="imgCell">
                                                            <span>
                    ...
                    <StyledCard elevation={6} className="echarts paddingbottom">
                        ...
                        <SimpleCard>
                            ...
                            <TabPanel style={{overflow:'auto'}}
                                ...
                                className="tabpanel nopadding notclear "
                            >
                                <div className="table_scroll scrollstyle" style={{width:'350%'}}>
                                    <StyledTable className="customtable odd-even withborder">
                                        <TableHead>
                                            <TableRow>
