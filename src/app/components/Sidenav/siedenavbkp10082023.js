...
const Sidenav = ({ children }) => {
    ...
    const [isMenuLoaded, setIsMenuLoaded] = useState(true)
    ...
    const getMenuList = async (searchvalue = "") => {
        ...
        var values=[];
        if(companyid){
         values = await getListDetails(companyid)
           
            console.log(values,"valuesss")
        }

        const others = (companyid) => {
            return {
                name: 'Others',
                iconText: 'SI',
                openinpoup: 1,
                mainMenu: companyid.companyName,
                children: [
                    {
                        name: 'Valuation',
                        iconText: 'SI',
        ...
        const updated = companylistmenu?.map(item => {
            if (item.companyid === companyid) {
                return { ...item, children: [...values.concat(shareholderRepository), others(item)] };
            } else {
                return { ...item, children: [...item.children, others(item)] };
            }
        });
        console.log(updated, "ppooppoopp")
        // const gg = (listValues) => {
        //     const filterVal = listValues.filter(o => o.name === 'Technology')
        //     const filterValOpen = listValues.filter(o => o.name === 'Opex')
        //     let pip = []
    ...
    const getListDetails = async (companyid) => {
        ...
        <Fragment>
            <StyledScrollBar options={{ suppressScrollX: true }} className='mysidebar' >
                <div className='mysidebar-div'>
                    ...
                    {isMenuLoaded && <MatxVerticalNav items={menulist}
